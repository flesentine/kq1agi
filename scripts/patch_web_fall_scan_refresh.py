...
read_repl = '''  let fallScanWatch = { room: -1, startedAt: 0 };\n\n  function readFallAccessibilityState() {\n'''
...
new = "const BUILD_TAG = '20260828-fall-accessible-v4-preload';"
if old in text:
    text = text.replace(old, new, 1)
elif new not in text:
    raise RuntimeError('FALL accessibility build tag not found for preload refresh')

path.write_text(text)
print('FALL preload UI installed: stale state repairs itself and stalled scans expose seeds + RETRY')
