...
new = "const BUILD_TAG = '20260828-fall-accessible-v5-worker';"
if old in text:
    text = text.replace(old, new, 1)
elif new not in text:
    raise RuntimeError('FALL accessibility build tag not found for worker refresh')

path.write_text(text)
print('FALL worker diagnostic UI installed: stalled scans expose game/editor room + seed state')
